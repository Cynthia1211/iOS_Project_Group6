...
// Author: Yue Zhang
// Description: Database helper class to handle reading and writing treasure and user score data in SQLite.
class TreasureManager {
    ...
    // Get active treasures near the user's location within a specific geographic range.
    func fetchTreasuresAround(centerLat: Double, centerLon: Double, halfSideLengthInMeters: Double) -> [Treasure] {
        
        // Create array to store the records get from database
        var treasures = [Treasure]()
        var db: OpaquePointer? = nil
        guard let path = databasePath else { return treasures }
        
        // Convert distance in meters into latitude/longitude degree range to filter records.
        // 1 degree of latitude is roughly 111,000 meters.
        // 1 degree of longitude is roughly 80,000 meters.
        let latDelta = halfSideLengthInMeters / 111000.0
        let lonDelta = halfSideLengthInMeters / 80000.0
        
        // Calculate the lat and lon range
        ...
        if sqlite3_open(path, &db) == SQLITE_OK {
            ...
            // Only fetch not found treasure and in required geographic range
            let querySQL = """
            ...
            if sqlite3_prepare_v2(db, querySQL, -1, &statement, nil) == SQLITE_OK {
                sqlite3_bind_double(statement, 1, minLat) // first ? in query statement
                sqlite3_bind_double(statement, 2, maxLat) // second ? in query statement
                sqlite3_bind_double(statement, 3, minLon) // third ? in query statement
                sqlite3_bind_double(statement, 4, maxLon) // fourth ? in query statement
                
                // Loops through each database record, wraps it into a Treasure object, and appends it to the treasures array.
    ...
    // Updates the treasure isTreasureFound and foundByUserId in the database
    func markTreasureAsFound(treasureId: String, foundByUserId: String) -> Bool {
        ...
        if sqlite3_open(path, &db) == SQLITE_OK {
            ...
            // Mark isTreasureFound = 1, treasureFoundby = user's UUID for the treasure being claimed
    ...
    // Adds earned points to user total score in database.
    func updateUserScore(userUUID: String, additionalPoints: Int) -> Bool {
        ...
        if sqlite3_open(path, &db) == SQLITE_OK {
            ...
            // Add new points onto existing score
    ...
    // Get treasures placed by a specific user.
    ...
    // Get treasures found by a specific user.
    ...
    // Insert a new treasure in database
